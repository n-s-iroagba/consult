      ...
      >
        <nav className="h-full overflow-y-auto p-3 lg:p-4 flex flex-col">
          ...
          <div className="mb-4 p-3 lg:p-4 border-b border-slate-700">
            ...
            <h2 className="mt-2 text-center text-base lg:text-lg font-semibold text-slate-100">Admin Dashboard</h2>
          </div>

          {/* Navigation Items - Improved mobile spacing */}
          <div className="flex flex-col gap-1">
            {[{ href: "/admin/dashboard", text: "Dashboard", icon: UserGroupIcon },
              { href: "/admin/attendants", text: "My Attendants", icon: UserGroupIcon },
              { href: "/admin/wallets", text: "My Wallets", icon: WalletIcon },
              { href: "/admin/events", text: "My Events", icon: UserCircleIcon },
              { href: "/admin/bank-details", text: "Bank Details", icon: LinkIcon },
            ].map((item, index) => (
              <Link
                key={index}
                href={item.href}
              ...
              >
                ...
                <span className="text-slate-100 font-medium text-sm lg:text-base truncate">{item.text}</span>
              </Link>
            ))}
                <Link
              href={''}
              ...
              >
                ...
                <span className="text-slate-100 font-medium text-sm lg:text-base truncate">Log out</span>
              </Link>
          </div>
        </nav>
      </aside>
